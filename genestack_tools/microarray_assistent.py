...
import patsy
import inmoose
from genestack_tools.llm_api import ask_model, T
...
class MicroarrayExpressionAssistent(Assistent):
    ...
    def initiate_adata(
        ...
        mask = pheno["source_name_ch1"].str.contains(group_pattern, case=False, na=False)
        mask &= pheno["source_name_ch2"] != exclude
        pheno_sub = pheno.loc[mask]
        expr_sub = expr[pheno_sub.index]

        groups = pheno_sub["source_name_ch1"].apply(lambda x: "I3C" if "I3C" in x else "DMSO")
        ...
        clinical = pd.DataFrame({"batch": batches, "group": groups}, index=expr_sub.columns)
    ...
    def run_limma(self, formula: str = "~ 0 + group", coef_name: str = "group[DMSO]", verbose: bool = True):
        if self.adata is None:
            print("No AnnData available. Run initiate_adata() first.")
            return None
